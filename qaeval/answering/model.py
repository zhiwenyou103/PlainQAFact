    ...
    def answer_all(
        ...
        for i, (question, context) in enumerate(input_data):
            if not question or not context:
                print(f"Question or context is empty for example {i}")
                print(f"Question: {question}")
                print(f"Context: {context}")
                question = context or "empty question"
                context = context or "empty context"
        ...
        for batch in generator:
            ...
            for i, feature_index in enumerate(feature_indices):
                ...
                if isinstance(outputs, dict):
                    start_logits = self._to_list(outputs['start_logits'][i])
                    end_logits = self._to_list(outputs['end_logits'][i])
                else:
